...
public class FlywayConfig {
    ...
    public Flyway flyway(DataSource dataSource, Environment env) {
        ...
        List<String> locations = Binder.get(env).bind("spring.flyway.locations", Bindable.listOf(String.class))
                .orElse(List.of("classpath:db/migration"));

        Flyway flyway = Flyway.configure().dataSource(dataSource).locations(locations.toArray(String[]::new)).load();
